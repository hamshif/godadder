...
- Start server for development/debug:
  ...
  - Equivalent CLI: `uvicorn --app-dir src startupper.startup_namer:app --host 127.0.0.1 --port 8000`.
...
- Breakpoints should bind in `src/startupper/startup_namer.py` (verify via Debug Console if needed).
